...
/// ############################################################################
    ...
    QString data_adapter_ODH::make_select_filter( const QString &s_filter ) const
    {
        ...
        s_where += "((instr(OBJECT_NAME,'" + s_filter + "')>0)OR";
        s_where += "(instr(OBJECT_NAME,'" + s_filter.toUpper( ) + "')>0)OR";
        s_where += "(instr(OBJECT_NAME,'" +
                ...
                data_adapter_ODH::select(const QString &s_filter/*=QString( )*/) const
	{
        ...
        data_violation_object_collection *obj_coll = 0;
        try
        {
